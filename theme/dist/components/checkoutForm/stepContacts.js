...
const ReadOnlyField = ({ step, name, value }) => {
	return React.createElement(
		'div',
		{
			className: step > 1 ? 'checkout__field-preview field-preview' : 'checkout__field-preview field-preview checkout__field-preview--stepcontacts'
		},
		React.createElement(
			'div',
	...
	render() {
		...
		if (isReadOnly) {
			return React.createElement(
				...
				React.createElement(
					'div',
					{ className: 'step__variants' },
					React.createElement(
						'h3',
						{ className: 'step__variants-title' },
					...
					React.createElement(
						'div',
						{ className: 'variants shipping-methods' },
						shippingMethods.map((method, index) => React.createElement(
							'label',
							{
								key: index,
								className: 'input__radio variants__item shipping-method' + (method.id === initialValues.shipping_method_id ? ' active' : '')
							},
							...
							React.createElement(
								'div',
								{ className: 'variants__label input__label' },
								React.createElement(
									'div',
									null,
									React.createElement(
										'div',
										{ className: 'variants__name' },
										method.name
									),
									React.createElement(
										'div',
										{ className: 'variants__description' },
										method.description
									)
								),
								React.createElement(
									'span',
									{ className: 'variants__rate' },
				...
				React.createElement(
					...
					React.createElement(
						'div',
						{ className: 'variants payment-methods' },
						paymentMethods.map((method, index) => React.createElement(
							'label',
							{
								key: index,
								className: 'input__radio variants__item payment-method' + (method.id === initialValues.payment_method_id ? ' active' : '')
							},
							React.createElement(Field, {
							...
							React.createElement(
								...
								React.createElement('span', { className: 'variants__logo payment-method__logo' })
							)
						))
					)
				)
			...
			return React.createElement(
				...
				React.createElement(
					'form',
					{ className: 'step__form', onSubmit: handleSubmit },
					...
					!this.isFieldHidden('address1') && React.createElement(Field, {
						...
						onBlur: (event, value) => setTimeout(() => saveShippingLocation({ address1: value }))
					}),
					!this.isFieldHidden('address2') && React.createElement(Field, {
						...
						onBlur: (event, value) => setTimeout(() => saveShippingLocation({ address2: value }))
					}),
					...
					!this.isFieldHidden('postal_code') && React.createElement(Field, {
						...
						onBlur: (event, value) => setTimeout(() => saveShippingLocation({ postal_code: value }))
					}),
